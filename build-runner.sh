...
# Build a new image
sudo docker build --tag $AWS_IMAGE_NAME$BUILD_IMAGE --file Dockerfile.queuerunner .
...
then
	...
	if ! sudo docker build -t $AWS_IMAGE_NAME$BUILD_IMAGE --file Dockerfile.queuerunner .
	then
